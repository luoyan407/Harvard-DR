...
newoctfile = '/shared/hdds_20T/yl535/elzelab/cirrus_ongoing/metadata_OpticDiscCube_ongoing_cleaned.csv.previous'
...
save_to_file = f'/shared/ssd_16T/yl535/project/python/datasets/harvard/glaucoma_table_3k_previous.csv'
save_to_folder = os.path.dirname(save_to_file)
save_file_name = os.path.basename(save_to_file)

seed = -1 #2740 # -1

log_output = os.path.join(save_to_folder, save_file_name+'.log')
log_output = None
...
pprint(f'# of VF records: {len(list(vfid_timetd.keys()))}', log_output, 'a')
...
#     bscanPixelspacingDepth = float(row['bscanPixelspacingDepth'])
    ...
    race = row['race']
    hisp = row['hispanic']
    if (np.isnan(race)) or (np.isnan(hisp)) or (int(race) not in dict_race): #  or (hisp not in ['0', '1'])
        continue
...
selected_data_df.to_csv(os.path.join(save_to_file), index=False)
